...
def _sync_map_with_live_data() -> None:
    """
    Keep map markers aligned with current bins live data.
    Route is only displayed if a route file exists (generated by button).
    """
    needs_refresh = not map_path.exists()
    if not needs_refresh:
        map_mtime = map_path.stat().st_mtime
        bins_mtime = bins_path.stat().st_mtime if bins_path.exists() else 0
        route_mtime = route_path.stat().st_mtime if route_path.exists() else 0
        needs_refresh = max(bins_mtime, route_mtime) > map_mtime
...
if route_meta_path.exists():
    try:
        route_meta = json.loads(route_meta_path.read_text(encoding="utf-8"))
    except Exception:
        route_meta = {}

has_snapshot_route = bool(route_meta) and route_snapshot_bins_path.exists()
...
if has_snapshot_route:
    st.caption(f"Trajectoire basee sur le snapshot du: {route_meta.get('snapshot_at', '-')}")
...
with action_col1:
    if st.button("Voir la trajectoire", use_container_width=True):
        with st.spinner("Calcul de la trajectoire..."):
            ...
            route_meta = {
                "route_id": route_id,
                "snapshot_at": pd.Timestamp.now(tz="UTC").strftime("%Y-%m-%d %H:%M:%S UTC"),
                "is_complete": bool(route_result.get("is_complete", False)),
                "missing_critical_bins": route_result.get("missing_critical_bins", []),
            }
            route_meta_path.write_text(json.dumps(route_meta, ensure_ascii=True, indent=2), encoding="utf-8")
            missing_after = route_meta["missing_critical_bins"]
            if missing_after:
                pd.DataFrame(
                    columns=[
                        "vehicle_id",
                ...
                st.error(
                    ...
                    "La trajectoire a ete invalidee."
                )
            else:
...
with action_col2:
    if st.button("Lancer collecte", type="primary", use_container_width=True):
        with st.spinner("Collecte en cours..."):
            if not route.empty and has_snapshot_route and bool(route_meta.get("is_complete", False)):
                ...
                st.info("Aucune trajectoire valide disponible. Clique d'abord `Voir la trajectoire`.")
        st.rerun()
